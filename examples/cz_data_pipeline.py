...
mouse_cell_type = "brain"
N = 200_000
cell_soma_ids = retrieve_soma_idx(mouse_cell_type, organism="mus_musculus")
adata = download_partition(0, mouse_cell_type, cell_soma_ids, N, "Mus musculus") 

# COMMAND ----------

if not check_vocab_exists(organism="mus_musculus"):
    create_base_vocab(adata, organism="mus_musculus")
else:
    update_base_vocab(adata, organism="mus_musculus")

# COMMAND ----------

# Only interested in the binned row
df = anndata_to_cell_bank_test(anndata=adata, cell_type=mouse_cell_type, output_directory=f"/Volumes/kvai_usr_gmahon1/thesis/test_data/{mouse_cell_type}")

# COMMAND ----------

kidney_data = spark.read.format("parquet").load(f"/Volumes/kvai_usr_gmahon1/thesis/test_data/brain/")
...
# ind_map = {x[0]: x[1] for x in mapped.select("index", "base_gene_id").collect()}
# # Testing adding binning to the map_indices_and_return_tokens code
# # Removed the mapping function until new vocab is built up for Mus musculus
# anndata = adata
# # ---------------------------------------------------------------
# n_rows, n_cols = anndata.X.shape
# new_indices = np.array(
